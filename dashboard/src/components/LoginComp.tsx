...
const LoginComp = (): JSX.Element => {
  ...
  const googleAuth = async (e: GoogleLoginResponse | GoogleLoginResponseOffline) => {
    ...
    <div>
      ...
      <form onSubmit={handleLogin}>
        <div className="mb-3">
          ...
          <input type="email" className="form-control py-2" value={info.email} onChange={(e) => setInfo({ ...info, email: e.target.value })} />
        </div>
        <div className="mb-3">
          <label htmlFor="exampleInputPassword1" className="form-label">
            Password
          </label>
          <input type="password" className="form-control py-2" value={info.password} onChange={(e) => setInfo({ ...info, password: e.target.value })} />
        </div>

        <button disabled={loading} className="btn btn-warning d-block w-100 text-white fw-bold py-2">
